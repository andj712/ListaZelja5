...
def save():
    ...
    if red.hgetall(username).keys():
        ...
        return render_template('index.html', user_exists=1, msg='(Из редиса)', username=username, place=red.hget(username,"mesto").decode('utf-8'), food=red.hget(username,"hrana").decode('utf-8'))
    ...
    # if not in redis, then check in db
        ...
        print("Записи преузети из базе:", record)
        ...
        if record:
            ...
            return render_template('index.html', user_exists=1, msg='(Из базе', username=username, place=record.place, food=record.food)
    ...
    print("Кључеви из редиса након инсерта:", red.hgetall(username))
...
def get():
	...
	if not user_data:
		...
		if not record:
			print("Нема података у бази и редису")
			return render_template('index.html', no_record=1, msg=f"Нису још увек дефинисани записи за {username}")
		red.hset(username, "mesto", record.place)
		red.hset(username, "hrana", record.food)
		return render_template('index.html', get=1, msg="(Из базе)",username=username, place=record.place, food=record.food)
	return render_template('index.html',get=1, msg="(Из редиса)", username=username, place=user_data[b'place'].decode('utf-8'), food=user_data[b'food'].decode('utf-8'))